...
## ADR-0030 — An owner is the voice that took the task, named or not

Status: accepted (2026-09-17)
Context: On real meetings nobody says anyone's name, so every speaker stayed unnamed, every owner came out `unassigned` and every quote was shown without a speaker. The screen read as "nobody took anything on", although the recording holds exactly who said "I'll do it". The gap does not depend on head count: ES2002d has two voices and no name.
Decision: When the model reports no owner, or one never spoken in the recording, the owner is the speaker of an acceptance quote in which they take the task on in the first person and without negation. That speaker is shown by name when known and otherwise as "speaker N — took it on, name not said", with status `unnamed_speaker`. Every quote carries the diarization label of its utterance, so a nameless quote still shows its voice. A plural or impersonal acceptance ("let's", "we're going to", "someone needs to") assigns nobody.
Consequence: Re-verified on the recordings that exposed the gap, quotes without a speaker fall from 8 of 8 to 0 of 8 on ES2002d and from 6 of 6 to 0 of 6 on IS1000a. The owners of ES2002d stay unassigned, correctly, because both of its commitments were accepted by the team in the plural. A model's invented owner is replaced by the voice that actually took the task, and `owner_not_a_known_name` still reports the invention.
Rejected: A participants field typed by the user was rejected because matching names to voices would still be a guess; letting the model infer the owner from role or context was rejected because the extraction prompt forbids exactly that guess, and the verified quote already carries the answer.
